...
def clean_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): DataFrame to clean column names for.
...
def drop_empty_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): DataFrame to drop empty rows from.

    Returns:
        pd.DataFrame: DataFrame with empty rows dropped.
    """
...
def transform_budget(df: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): DataFrame to transform.
        label (str): Label for the transformed DataFrame.
