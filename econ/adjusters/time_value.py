...
def discount_factor_series(
    ...
    *,
    ...
    if mode in ("series", "fisher") and rate_series is not None and not rate_series.empty:
        ...
        rs = rs.sort_index()
        factors = []
        nom_rate = float(annual_rate or 0.0)
        for ts in t:
            if pd.isna(ts):
            ...
            acc = 1.0
            for m in months:
                infl = float(rs.get(m, np.nan))
                if np.isnan(infl):
                    infl = float(rs.ffill().bfill().reindex([m]).iloc[0])
                
                # If mode is series, the series provides the nominal rate directly.
                # If mode is fisher, the series provides inflation, and we calculate real rate.
                if mode == "fisher":
                    # Fisher equation: (1 + r) = (1 + i) / (1 + pi)
                    rr = ((1.0 + nom_rate) / (1.0 + (infl / 100.0 if infl > 1 else infl))) - 1.0
                else:
                    rr = infl
